...
// Вспомогательная функция: делает такой же POST-запрос в MAX, как раньше делал fetch(),
// но с явно подсунутыми правильными корневыми сертификатами Минцифры,
// поэтому TLS проверяется НЕ отключается, а проходит по-настоящему.
function sendToMax(chatId, token, text) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (MAX_BOT_TOKEN && MAX_CHAT_ID) {
      try {
        const maxResult = await sendToMax(MAX_CHAT_ID, MAX_BOT_TOKEN, plainText);
        if (maxResult.status < 200 || maxResult.status >= 300) {
          console.error("MAX API Error:", maxResult.status, maxResult.data);
        }
      } catch (err) {
